...
class Lead {
  ...
  `;
    ...
    try {
      ...
      console.error('   Parâmetros:', params.map((p, i) => `${i + 1}: ${p}`).join(', '));
  ...
  async update(updates = {}) {
    ...
    // Recalcular score e tags se necessário
  ...
  static async findById(id) {
    const query = 'SELECT * FROM teste.leads WHERE id = $1';
  ...
  static async findByEmailId(emailId) {
    const query = 'SELECT * FROM teste.leads WHERE email_id = $1';
  ...
  static async getDashboardStats(dataInicio, dataFim) {
    ...
    return {
      ...result,
      taxaConversao: result.total_leads > 0
  ...
  static async searchAdvanced({ filters = {}, page = 1, limit = 50, sortBy = 'dataRecebimento', order = 'DESC' }) {
    const db = require('../config/database');
  ...
  static toCSV(leads) {
    const headers = [
      'ID', 'Nome', 'Email', 'Telefone', 'Veículo de Interesse',
      'Origem', 'Status', 'Prioridade', 'Score', 'Data Recebimento',
      'Data Contato', 'Vendedor ID', 'Tags'
    ];
